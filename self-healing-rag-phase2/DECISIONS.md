...
## D-007: Injectable embedding function in VectorStore
Decision: `VectorStore.__init__` takes an `embed_fn` parameter defaulting to
the real `embed_texts` (sentence-transformers).
Rationale: This is dependency injection for testability, not a provider
abstraction — there's still exactly one production embedding path. It lets
unit tests use a deterministic, network-free fake embedder instead of
downloading real model weights on every test run, which is standard
practice regardless of environment constraints.
Alternatives Considered: Hardcoding `embed_texts` directly inside
`VectorStore` — rejected, would force all vector-store tests to depend on
network access and a multi-hundred-MB model download.
Date: Phase 2
